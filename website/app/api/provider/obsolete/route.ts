...
export async function POST(request: Request) {
  ...
  try {
    const result = await db
      ...
      .set({ obsoleteCount: sql`obsoleteCount + 1` })
    ...
    const response = formatResponse(result, 'Obsolete count incremented successfully')
    return Response.json(response)
  }
  catch {
    const response = formatResponse({}, 'Failed to increment obsolete count', 1)
    return Response.json(response)
  }
}
